...
```{r, eval=FALSE}
dat <- data.frame(plot = 1:50, height = rnorm(1000, 10, 10)) # Generate dummy dataset
plots <- unique(dat$plot)                            # first we need a list of each plot, with no repeats. 

for(ii in 1:length(plot)){                       # tell the for-loop to run the same number of times as the number of plots we have
  a <- subset(dat, dat$plot == ii)               # subset the data.frame "dat" to include only rows where plot matched "ii"
  b <- mean(a$height)                            # find the mean height for plot "ii"
  c <- data.frame(plot = dat$plot[ii], mean = b) # put the mean height and plot id into a data.frame
...
* `apply` functions actually use for-loops "under the hood" i.e. you'll see for loops if you look at the source code
    * Base R apply functions are written in C; writing code in C, then calling it from R, happens in a lot of functions and can be faster in a lot of cases
* __Old Perception__: "for loops in R are slow and should be avoided""
* __Today__: for loops can be just as "performant" or fast in R now, and were optimized many years ago
...
* `plyr` has similar functionality to Base R apply functions with
    ...
    * Simple function grammar ("ddply"" = data frame input, data frame output; "ldply"" = list input, data frame output; "dlply"" = data frame input, list output etc.)
    * Quick multivariate/highly nested processing e.g. how fun will it be to write a nested for loop to summarize a data set with 5 factor columns and one response variable?
