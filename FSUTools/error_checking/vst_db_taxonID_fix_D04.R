...
wd <- paste("C:/Users/cflagg/Documents/Test/vst_fixes/D04/", sep="")
...
for (domain in unique(domainList)){
  ...
  for (files in unique(vst_files)){
    ...
    if ("taxonID" %in% colnames(vst_dat)){
      ...
      # print file
      ### split file path, count the number of splits, then grab the last split (i.e. the file name)
      all_str_out <- str_split(files,"/") # split the file name
      lastPiece <- length(all_str_out[[1]]) # count how many pieces there are, pass to next line
      name_out <- all_str_out[[1]][[lastPiece]] # grab the name of the file
...
# test <- "C:/Users/cflagg/Documents/Test/vst_fixes/D04/vst_shrubgroup_in.csv"
